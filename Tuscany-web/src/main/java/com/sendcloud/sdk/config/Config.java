...
public class Config {
	...
	// 邮件user
	public static String api_user = "draper_hxy_test_C8Kxtm";
	// 邮件key
	public static String api_key = "8HjD2VEVMXScKD0n";
	// 短信user
	...
	static {
		try {
			InputStream f = Config.class.getClassLoader().getResourceAsStream("config/api.properties");
			Properties pros = new Properties();
			pros.load(f);
			send_api = pros.getProperty("sendcloud.send_api");
			send_template_api = pros.getProperty("sendcloud.send_template_api");
			send_sms_api = pros.getProperty("sendcloud.send_sms_api");
			send_voice_api = pros.getProperty("sendcloud.send_voice_api");
			api_user = pros.getProperty("sendcloud.api_user");
			api_key = pros.getProperty("sendcloud.api_key");
			sms_user = pros.getProperty("sendcloud.sms_user");
			sms_key = pros.getProperty("sendcloud.sms_key");
			f.close();
		} catch (Exception e) {
			e.printStackTrace();
		}
